...
//       tail=new Node((LL)0xffffffffffffffff);
      ...
      new (tail) Node((LL)0xffffffffffffffff);
#else
//       tail=new Node((LL)0xffffffff);
      tail = (Node *) malloc(sizeof(Node));
      new (tail) Node((LL)0xffffffff);
#endif
...
extern "C" void Enqueue(int i) {
    sl.Add(i,NULL);
}

extern "C" int Dequeue() {
    Node* n=sl.FindAndMarkMin();
    if (n != NULL) {
        sl.Delete(n->key);
        return n->key;
    } 
